...
def plot_stuff(
    raw1, raw2, raw3, raw4, denoised1, denoised2, denoised3, denoised4, denoised_bin
):
    fig = plt.figure()
    ...
    df_out = pd.DataFrame(
        {
            ...
            "AI2 (V)": list4,
            "AI3 (V)": list5,
            "channel_binary": list6,
        }
    )
    df_out.to_csv(
        r"D:\Coding\Year 3\group_project\done\ce\Nic, Flexion, Dry CE, 1 filtered.csv"
    )
...
df = pd.read_csv(
    r"D:\Coding\Year 3\group_project\unfiltered\Ivan\Ivan S, Flexion, Regular Electrodes, 3.csv",
    usecols=lambda x: x in collumns,
    header=6,
)
